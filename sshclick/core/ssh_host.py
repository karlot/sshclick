...
class HostType(str, Enum):
    """Known host entry kinds used by parsed SSH config data."""
...
class SSH_Host:
    ...
    def get_source_label(self) -> str:
        """Return a `file:line` label when source provenance is available."""
    ...
    def __rich__(self):
        """Delegate Rich rendering to the selected host-style module."""
        try:
            # If current host "print_style" is set to "panels", we will try
            # to import module from "./host_styles/panels.py"
            style = importlib.import_module(f'sshclick.core.host_styles.{self.print_style}')
