...
@click.option("--config", default=USER_SSH_CONFIG, envvar="SSHC_SSHCONFIG", help=SSHCONFIG_HELP)
...
def cli(ctx: click.core.Context, config: str, stdout: bool, diff: bool):
    ctx.obj = SSH_Config(file=os.path.expanduser(config), stdout=stdout, diff=diff).read().parse()
